...
def classify(sender, data, user_data):
    ...
    if model_loaded:
        if img_path == "resources/def_img.jpeg":
...
def test_cnn(model, file_path, im_size = 496):
    ...
    if out.argmax() == 0:
        ...
        diagnosis = 'Diabetic Macular Edema (score = ' + str(round(100.0*out[0][1], 2)) + ' %).'
    elif out.argmax() == 2:
...
dpg.create_context()
dpg.create_viewport(title="Retinal Diseases Detection", width=831, height=410)
...
# OCT image window
with dpg.window(label="OCT Image",width=315, height=370, pos=(500,0),no_resize=True,no_collapse=True, no_close=True, no_move=False):
    dpg.add_image(load_image("resources/def_img.jpeg"), tag='cto_image', width=300, height=300)
